...
<head>
  ...
  <title>Teras Menull | Roti Menull Knk</title>
...
<body data-category-page="teras-menull">
  <script>
  ...
  <main>
    <section class="section menu-categories" id="menu">
      <div class="section-header">
        <h2>Teras Menull Menu</h2>
        <p>Snack dan teman nongkrong terbaik ala Teras Menull.</p>
      </div>
  ...
  <script>
    document.addEventListener('DOMContentLoaded', async () => {
      ...
      // Load Teras Menull (Prasmanan) products
      const allProducts = await loader.loadProducts();
      loader.products = allProducts.filter(p => p.brand_id === 3);
      ...
      function updateCartPreview() {
        ...
        list.innerHTML = cart.map((item) => {
          const variantNames = (item.packageItemNames && item.packageItemNames.length)
            ? item.packageItemNames
            : (item.selectedVariantNames || []);
          const variantLine = variantNames.length
            ? `<div class="preview-item-variant">${variantNames.join(', ')}</div>`
            : '';
          return `
          <div class="preview-item">
            ...
            <div class="preview-actions">
              <span style="white-space: nowrap">${EnhancedCartSystem.formatCurrency(item.subtotal)}</span>
              <button class="preview-remove" type="button" data-remove-id="${item.id}" aria-label="Hapus pesanan">−</button>
            </div>
          </div>
        `;
        }).join('');
